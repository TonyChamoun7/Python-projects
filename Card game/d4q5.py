...
def donne_cartes(p):
     ...
     #####################
     # Le donneur donne une carte à l'autre joueur, une à lui-même, continue jusqu'à la fin du paquet
     x = 1
...
def elimine_paires(l):
    ...
    liste = []
    liste2 = []
    # Elimine toutes les paires de la liste
...
def affiche_cartes(p):
    ...
    #####################
# Affiche les éléments de la liste séparées par des espaces
    s2 = ''
    for ch in p[:len(p)-1]:
        s2 += ch + ' '
    s2 += p[len(p)-1]
...
def entrez_position_valide(n):
     ...
     #####################
     # Permet a l'usager d'entrer un entier
     # Affiche differentes messages selon l'entier que l'usager a entrer
     if n == 1:
...
#####################
# Alternateur entre l'humain et le robot, permet le changement de tour
def alternate(d, h, x):
    if x == 0:
        print("***********************************************************")
        print("Votre tour.")
...
# Elimine la carte que l'usager a demander d'eliminer
def eliminate(l, h, y):
    x = l[y-1]
    h.append(x)
    l.remove(x)
...
# Toutes les fonctions sont ajoutées ici pour que le jeu puisse se jouer
     x = 1
     while x ==1:
         b = 1
